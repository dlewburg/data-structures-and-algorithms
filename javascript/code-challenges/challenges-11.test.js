...
const count = (target, input) => {
  // Solution code here...
  const count= input.reduce((accum, curVal) => {
    const subCount = curVal.filter(item => item === target).length;
    return accum + subCount;
  }, 0);
  return count;
};
...
const totalSum = (input) => {
  ...
  let totalSum = flatArr.reduce((accum, curVal) => accum +curVal, 0);
  return totalSum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filterArr = input.map(subArr => {
    let inArr = subArr.filter(item => typeof item === 'number' && item % 5 === 0);
    let pwrArr = inArr.map(item => Math.pow(2, item));
    return pwrArr;
  });
  return filterArr;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let character = data.filter(char => char.gender === 'male' || char.gender === 'female');
  let names = character.map(char => char.name);
...
let findShortest = (data) => {
  // Solution code here...
  let shortChar = data.filter((character) => character.height > 0);
  shortChar.sort((a,b) => +(a.height) - +(b.height));
  return shortChar[0].name;
};
