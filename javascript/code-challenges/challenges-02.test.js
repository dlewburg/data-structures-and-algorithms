...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let thirdPower = arr.map(pwr => Math.pow(pwr, 3));
  return thirdPower;
...
const addQuestion = (arr) => {
  // Solution code here...
  let addPunct = arr.map(punct => punct + '?');
  return addPunct;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    forArray.push(Math.pow(2, arr[i]));
  }
  return forArray;
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let arrLoop = [];
  arr.forEach(loop => arrLoop.push(Math.pow(2, loop)));
  return arrLoop;
};
